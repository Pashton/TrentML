...
def load_faces():
    ...
    train_set_x, train_set_y = shared_dataset([X[:-100],Y[:-100]])
...
def load_caltech101():
    ...
    X_test = []
    Y_test = []
    for root, dirs, files in os.walk(os.path.abspath('101_ObjectCategories')):
        count = 0
        for file in files:
            if str(file).endswith('.jpg') :
                ...
                data = numpy.asarray(im.getdata())
                mini = float(min(data))
                maxi = float(max(data))
                data = (data-mini) / float((maxi - mini))
                if count < 30 :
                    count = count + 1
    ...
    rval = [(train_set_x, train_set_y), (valid_set_x, valid_set_y),
            (test_set_x, test_set_y)]
    return rval
def load_alph(ds, seed, dtype=None):
    X = []
    Y = []
    X_test = []
    Y_test = []
    def walk(ds) :
        X=[]
        Y=[]
        for root, dirs, files in os.walk(os.path.abspath(ds)):
            count = 0
            for file in files:
    ...
    if dtype == None :
        X,Y=walk(ds)
        X_train, X_test, Y_train, Y_test = train_test_split(X, Y, train_size=0.5, random_state=seed)
    else :
        X_train, Y_train = walk(ds)
        X_test, Y_test = walk('Bmp')
    assert len(numpy.unique(Y_train)) > 61
    le = preprocessing.LabelEncoder()
    le = le.fit(Y_train)
...
def load_lfw():
    ...
    X_test = []
    Y_test = []
    for root, dirs, files in os.walk(os.path.abspath('lfw2')):
        count = 0
        for file in files:
            if len([name for name in os.listdir(os.path.join(root))]) < 6 :
                break
            if str(file).endswith('.jpg') :
                count = count + 1
                im = Image.open(os.path.join(root, file))
...
def load_cifar():
    ...
    train_set_y = []
    for x in range(1,5) :
        cifar = unpickle(os.path.join(os.getcwd(),'data_batch_')+str(x))
        train_set_x.extend(cifar['data'])
        train_set_y += cifar['labels']
    cifar = unpickle('test_batch');
